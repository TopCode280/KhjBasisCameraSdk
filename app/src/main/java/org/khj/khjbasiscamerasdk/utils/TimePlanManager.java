...
public class TimePlanManager {
    ...
    public ArrayList<String> transformToString(ArrayList<TimeSlot> timeSlots) {
        ...
        for (int i = 0; i < timeSlots.size(); i++) {
            ...
            if (temStart > temStop) {
                stopTime = stopTime + App.context.getString(R.string.nextDay);
            }
    ...
    public int updateTask(String task, int index) {
        ...
        ViseLog.e("输入合法吗" + task + m.matches());
        if (!m.matches()) {
            throw new IllegalArgumentException("输入的时间格式不对");
        }
     ...
     * @param plan
     */
    public void fromPlan(String plan) {
        ViseLog.i(plan);
        if (CommonUtil.isNull(plan)) {
            ViseLog.i("无计划");
            return;
        }
        String[] strings = plan.split("\n");
        for (String string : strings) {
            TimeSlot timeSlot = getTimeSlotByPlan(string);
            slotArrayList.add(timeSlot);
...
//        KLog.e(builder.toString());
    ...
    public static TimeSlot getTimeSlotByPlan(String string) {
...
//            KLog.w("close" + TimeUtil.long2String(close, "yyyy-MM-dd HH:mm:ss"));
        ...
        return new TimeSlot(0, close, open);
    }

    public static String slotToPlan(TimeSlot timeSlot) {
        long closeMills = timeSlot.startTime;
        long openMills = timeSlot.endTime;
        String close = closeMills / 1000 + "";
        String open = openMills / 1000 + "";
        ...
        String timePlan = builder.toString();
        ViseLog.i("slotToPlan:" + timePlan);
        return timePlan;
    }
